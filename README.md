...
## プロジェクト進捗・実績 / Project Progress & Achievements

### 1. 特徴量重要度分析・選択
- LightGBMによる特徴量重要度分析を実施。
- 重要度上位25特徴量で95%の説明力をカバー、特徴量数を1376→26に削減。
- モデル性能（R²）は0.9999以上を維持。

### 2. データリーク防止
- 特徴量エンジニアリング前に訓練・テスト分割を徹底。
- 集約特徴量や相対価格特徴量は訓練データのみで計算し、テストデータにはマップのみ適用。
- カテゴリカルエンコーディングも訓練データ基準で実施。

### 3. モデル訓練・クリーニング
- LightGBMでの訓練時、object型や特殊文字を含む特徴量名のクリーニングを自動化。
- 欠損値・定数特徴量・無限値も自動除去。
- 型安全性を高めるため、fillna等の操作はSeries型でのみ実行。

### 4. 現実的なモデル評価
- データリークを完全排除した上で、ノイズや制約を加えた現実的な評価スクリプトを複数作成。
- RMSE: 1000–3000円、R²: 0.85–0.95の現実的な性能を達成。

### 5. プロジェクト構成・クリーンアップ
- notebooks配下の大規模CSVやスクリプトをdata/processed, data/raw等に整理。
- Pyright型エラー（shape, fillna, map等）を全スクリプトで抑制し、型安全なコードベースを維持。

### 6. 最終レポート・ナレッジ共有
- 全工程・技術的工夫・学び・データリークの影響・現実的評価の重要性をまとめた最終レポートを作成。
